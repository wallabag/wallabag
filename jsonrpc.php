...
// Add a new link
$server->register('item.add', function ($url) {

    return Model\add_link($url);
});
...
$server->register('item.count_unread', function () {
    ...
    return Model\count_entries('unread');
});
...
$server->register('item.count_read', function () {
    ...
    return Model\count_entries('read');
});
...
$server->register('tag.item.list', function ($tag_id) {

    return Model\get_entries_by_tag($tag_id);
});
