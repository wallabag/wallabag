<?php if (empty($items)): ?>
    <p class="alert alert-info"><?= t('No entry for this tag') ?></p>
<?php else: ?>

    <div class="page-header">
        <h2><?= t('Tag') ?> <?= $tag['value'] ?> (<?= $nb_items ?>) <a href="<?= Helper\get_current_base_url().'feed.php?token='.urlencode($feed_token) . '&amp;status=tag&amp;value='.$tag['id'] ?>" >rss</a></h2>
    </div>
