...
class Url {
    ...
    public static function extract($url) {
        ...
        $extractor = new \ContentExtractor(
            dirname(__FILE__).'/../Readability/site_config/custom',
            dirname(__FILE__).'/../Readability/site_config/standard'
        );
        
        $extractor->fingerprints = $fingerprints;

        $http = new \HumbleHttpAgent();
        $http->userAgentMap = $user_agents;
        $http->headerOnlyTypes = array_keys($content_type_exc);
        $http->rewriteUrls = $rewrite_url;
        ...
        foreach ($items as $key => $item) {
            ...
            if ($permalink && ($response = $http->get($permalink, true)) && ($response['status_code'] < 300 || $response['status_code'] > 400)) {
                ...
                if (preg_match('!^Content-Type:\s*(([-\w]+)/([-\w\+]+))!im', $response['headers'], $match)) {
                    ...
                    foreach (array($match[1], $match[2]) as $_mime) {
                        if (isset($content_type_exc[$_mime])) {
                            $type = $match[1];
                            $_act = $content_type_exc[$_mime]['action'];
                            $_name = $content_type_exc[$_mime]['name'];
                            if ($_act == 'exclude') {
                ...
                if ($do_content_extraction) {
                    ...
                    $html = self::convert_to_utf8($html, $response['headers']);

                    // check site config for single page URL - fetch it if found
                    if ($single_page_response = self::getSinglePage($item, $html, $effective_url)) {
                        $html = $single_page_response['body'];
                        // remove strange things
                        $html = str_replace('</[>', '', $html); 
                        $html = self::convert_to_utf8($html, $single_page_response['headers']);
            ...
            if ($do_content_extraction) {
                ...
                if (!$extract_result) {
                    ...
                    if ($rewrite_relative_urls) self::makeAbsolute($effective_url, $content_block);
    ...
    private static function convert_to_utf8($html, $header=null)
    {
        $encoding = null;
        if ($html || $header) {
    ...
    private static function makeAbsolute($base, $elem) {
        ...
        foreach(array('a'=>'href', 'img'=>'src') as $tag => $attr) {
            ...
            for ($i = $elems->length-1; $i >= 0; $i--) {
                ...
                self::makeAbsoluteAttr($base, $e, $attr);
            }
            if (strtolower($elem->tagName) == $tag) self::makeAbsoluteAttr($base, $elem, $attr);
        }
    }

    private static function makeAbsoluteAttr($base, $e, $attr) {
    ...
    private static function makeAbsoluteStr($base, $url) {
        $base = new \SimplePie_IRI($base);
    ...
    private static function getSinglePage($item, $html, $url) {
        global $http, $extractor;
        ...
        if (isset($splink)) {
            ...
            if (isset($single_page_url) && ($single_page_url = self::makeAbsoluteStr($url, $single_page_url))) {
