...
Thanks to this documentation, we'll see how to interact with the
wallabag API.
...
As we work on a fresh wallabag installation, we'll have no result with
this command:
...
Now, if you execute the previous command (see **Get existing entries**),
you'll have data.
...
And if you want to list the existing entries (see **Get existing
entries**), the array is empty.
...
Have a look on the listing here: <http://localhost:8000/api/doc> to know
each method.
